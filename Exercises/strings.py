...
def remove_special_characters(string_to_clean):
    return string_to_clean.replace("-", "").replace("@", "").replace("\\", "").replace("/", "").replace("!", "").replace(" ", "")


print(remove_special_characters("@Hello- /Everyone!"))
...
print(("Passed ✅" if test == "Hello World" else "Failed ❌") +
      " Test 2 (remove_whitespace)")

test = remove_special_characters("@Hello- /Everyone!")
print(("Passed ✅" if test == "HelloEveryone" else "Failed ❌") +
      " Test 3 (remove_special_characters)")
...
print(("Passed ✅" if test == {"B": 1, "a": 3, "n": 2}
      else "Failed ❌") + " Test 5 (count_character_occurrences)")
